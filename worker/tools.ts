  ...
  return {
    ...
    web_search: {
      ...
      execute: async ({ query }: { query: string }) => {
        try {
          ...
          // In production, consider a dedicated SERP API or robust selectors
    ...
    generate_image: {
      ...
      execute: async ({ prompt }: { prompt: string }) => {
        ...
        // Return a markdown image link with the base64 data
    ...
    start_deep_research: {
      description: "Start a long-running, deep research workflow on a complex topic. Use this for reports.",
      parameters: z.object({
      ...
      execute: async ({ topic }: { topic: string }) => {
        ...
        const run = await env.RESEARCH_WORKFLOW.create({
          params: { 
            topic, 
            agentId: agent.id.toString(), // Pass Agent ID so workflow can reply back
            connectionId 
          }
        });
